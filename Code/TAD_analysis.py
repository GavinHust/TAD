...
def TAD_analysis(mode):
    if mode=="ER":
        # 生成ER网络的瓦解数据
        dir = "../Data/Synthetic/ER/"
        target_dir = '../Data/DNdata/NPY/Synthetic/ER/'
        file_pre = "ER_1000_"
    elif mode=="SF":
        # 生成SF网络的瓦解数据
        dir = "../Data/Synthetic/SF/"
        target_dir = '../Data/DNdata/NPY/Synthetic/SF/'
        file_pre = "SF_1000_"
    elif mode=="F_ER":
        # 生成不同F值的ER网络的瓦解数据
        dir = "../Data/Synthetic/F/ER/"
        target_dir = '../Data/DNdata/NPY/Synthetic/F/ER/'
        file_pre = ""
    elif mode=="F_SF":
        # 生成不同F值的SF网络的瓦解数据
        dir = "../Data/Synthetic/F/SF/"
        target_dir = '../Data/DNdata/NPY/Synthetic/F/SF/'
        file_pre = ""

    elif mode == "real":
        # 真实网络的瓦解数据
        dir = "../Data/Real/"
        target_dir = '../Data/DNdata/NPY/Real/'
        file_pre = ""
    else:
        print("error")
    ...
    for file in filenames:
        ...
        # MinSum方法取节点
        print("MiniSum")
        idx1 = np.load('../Data/Otherdata/SM_selected/' + file + '-output.npy').astype(int)
        ...
        # FINDER方法取节点
        print("FINDER")
        idx2 = np.load('../Data/Otherdata/FD_selected/' + file + '.npy').astype(int)
        #nodes = np.array(list(g.nodes))[idx2]
        #idx2 = nodes.tolist() + list(set(g.nodes()) - set(nodes))
        ...
        np.save(target_dir + filename + '_back.npy', strong_list)
        np.save(target_dir + filename + '_rand.npy', strong_list_rand)
        np.save(target_dir + filename + '_adpDegree.npy', strong_list_adapt_degree)
        np.save(target_dir + filename + '_degree.npy', strong_list_degree)
        np.save(target_dir + filename + '_MS.npy', strong_list_MS)
        np.save(target_dir + filename + '_PR.npy', strong_list_pagerank)
        np.save(target_dir + filename + '_DND.npy', strong_list_DND)  # 训练集只有SF网络
        if mode == "real":
            np.save(target_dir + filename + '_finder.npy', strong_list_FINDER)
        else:
            np.save(target_dir + filename + '_FD.npy', strong_list_FINDER)
        np.save(target_dir + filename + '_Core.npy', strong_list_Core)
        np.save(target_dir + filename + '_control.npy', strong_list_control)
...
mode=="ER"  # 生成ER网络的瓦解数据
mode=="SF"  # 生成SF网络的瓦解数据
mode=="F_ER"  # 生成不同F值的ER网络的瓦解数据
mode=="F_SF"   # 生成不同F值的SF网络的瓦解数据
mode=="real"   # 真实网络的瓦解数据
"""
mode = "real"
TAD_analysis(mode)
